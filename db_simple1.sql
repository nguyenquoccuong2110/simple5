...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th4 28, 2020 lúc 03:53 AM
...
(2, 'Lò Vi Sóng', '20MX63-L', '20 Lít', 'Hoạt động với 6 mức công suất.', 'Nấu / Hâm / Nướng / Rã đông.', 'Có', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', 3);
...
CREATE TABLE `users` (
  ...
  `datebirth` varchar(10) COLLATE utf8_unicode_ci NOT NULL,
  `monthbirth` varchar(10) COLLATE utf8_unicode_ci NOT NULL,
  `yearbirth` varchar(10) COLLATE utf8_unicode_ci NOT NULL,
...
--
ALTER TABLE `users`
  ADD CONSTRAINT `users_ibfk_1` FOREIGN KEY (`email`) REFERENCES `db_banhang1`.`users` (`email`);
COMMIT;
